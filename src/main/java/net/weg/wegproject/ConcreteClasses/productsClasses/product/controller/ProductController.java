...
public class ProductController {
    ...
    public ResponseEntity<List<Product>> searchBy(@PathVariable String searchTerm,
                                                  @RequestParam("size") int size,
                                                  @RequestParam("page") int page) {
        try {
            System.out.print(searchTerm);
            return ResponseEntity.ok(productService.searchBy(searchTerm, PageRequest.of(page, size)));
        } catch (Exception e) {
            throw new NoProductException();
        }
    }
